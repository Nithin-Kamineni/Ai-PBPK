...
pred.nano <- function(pars, pred=FALSE){
  ...
  BW           = pars["BW"]/1000  # Body weight; unit: kg; this value can use default
  PDOSEiv      = pars["Dose"]     # input dose; unit: mg/kg; this value can get from user input
  TDOSE        = 1                # total number of dose; this value can get from user input
  tinterval    = 24               # dose interval; unit is hours; this value can get from user input
  DOSEiv       = PDOSEiv * BW     # total dose; unit is mg; 
  tt           = 0.1              # time interval; 
  # define the exposure regimes; different exposure routes
  ex.iv.1 <- ev( ID = 1, amt= DOSEiv,                ## Set up the exposure regimes; now only iv is considered
  ...
  tsamp  = tgrid(0, tinterval*(TDOSE - 1) + 180, tt=0.1)
  ...
  # output the time-varying data -- time vs. tumor delivery efficeincy (%ID)
  outdf = cbind.data.frame (Time    = out$time, 
                            DETumor  = (out$Tumor/DOSEiv)*100)
  
  if(pred) return(outdf)
  
  # output the tumor delivery efficiency at 24, 168 and maximum DE
...
# Define the parameters; these parameter need to be inputed from user...
...
# Get the time-varying predictions
pred_Dat = pred.nano(pars, pred=TRUE)%>%mutate(DETumor_g=DETumor/TW)%>% select(Time, DETumor_g)
print(pred_Dat)
p <-ggplot(data=pred_Dat) +
    geom_line(aes(Time, (DETumor_g)),linetype="twodash", size=1)+
