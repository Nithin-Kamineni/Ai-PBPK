...
function(req) {
  ...
  list(message = "JSON object received and printed",result=req$body$a)
}
...
function(req) {
  # Get the JSON content from the request
  print(req$body$predKmax)
  
  PredKmax = req$body$predKmax
  Data_1 <- read.csv("Data_1.csv")
  
  R.Kmax = as.data.frame(PredKmax)
  head(R.Kmax)
  
  # Print the received JSON object
  df_pars <- R.Kmax%>%select(
    ID = ID,
    KTRES_n   = KTRESn,
    KTRES_max  = KTRESmax,
    KTRES_50   = KTRES50,
  ...
  d <- pred_all_2 %>% filter(ID <150 & ID >133)
  ...
  p5<-ggplot(data=d) +
    ...
    theme (
      ...
      text              = element_text (family = "Calibri"),   # text front (Time new roman)
  ...
  #list(message = "JSON object received and printed",result="dwd")
  # Save the plot as a temporary image file
  temp_image_file <- tempfile(fileext = ".png")
  ggsave(temp_image_file, plot = p5, width = 8, height = 6)
  
  # Read the image file as binary data
  image_binary <- readBin(temp_image_file, what = "raw", n = file.info(temp_image_file)$size)
  
  # Delete the temporary image file
  unlink(temp_image_file)
  
  # Create a response list with the plot image
  response <- list(
    message = "Plot image generated",
    plot_image = image_binary,
    list = list(d)
  )
