...
function(req) {
  ...
  print('edddddddddddddddddddddddddddddddd')
  
  PredKmax = req$body$predKmax
  Data_1 <- read.csv("Data_1.csv")
  
#   R.Kmax = as.data.frame(PredKmax)
pre_pars = as.data.frame(PredKmax)

EXP_Data_1=Data_1 [1,]
EXP_Data_1

Dat <- EXP_Data_1 %>% select(
     QTC = QTC,
     VTC = VTC,
     BVT = BVT,
     PT = PT,
...
m_df_pars <- cbind (pre_pars, Dat)
m_df_pars

pars <- c(
    BW          = req$body$BW,  #0.3, # input from user
    Dose         = req$body$Dose, #0.5,  # input from user
    KTRESn        = m_df_pars[3],
    KTRESmax       = m_df_pars[2],
    KTRES50        = m_df_pars[1],
    KTRESrelease     = ifelse(m_df_pars[4]<0, 0.005, m_df_pars[4]), # replace the negative value
    QTC          = m_df_pars[5],
    VTC          = m_df_pars[6],
    BVT          = m_df_pars[7],
    PT           = m_df_pars[8],
...
pred_Dat = pred.nano(pars, pred=TRUE) %>% mutate(DETumor)%>% select(Time, DETumor)
